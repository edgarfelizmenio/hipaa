<table class="parent-panel">
<tr>
<td class="left-panel-header">
<div class="left-panel-topbar">
<button id="button_compose_" name= "compose_" class="left-panel-topbar-buttons" onclick="window.location = &quot;index.php?tab=mail&amp;target=compose&quot;;">Compose New Message</button>
</div>
</td>
<td class="msg-panel-header">
<div class = "horizontal-nav">
<ul>
<li class="active"><a href = "index.php?tab=mail">Mail</a></li>
<li><a href = "index.php?tab=tour">Product Tour</a></li>
<li><a href = "index.php?tab=guide">HIPAA Guide</a></li>
<li><a href = "index.php?tab=preferences">Preferences</a></li>
</ul>
</div>
</td>
</tr>
<tr class="main-panel">
<td class="left-panel">
<div class="left-panel-content-area">
<div class="verticle-nav">
<ul>
<li class="active"><a href="index.php?tab=mail&amp;target=inbox">Inbox</a></li>
<li><a href="index.php?tab=mail&amp;target=sent">Sent</a></li>
<li><a href="index.php?tab=mail&amp;target=draft">Draft</a></li>
<li><a href="index.php?tab=mail&amp;target=thrash">Thrash</a></li>
</ul>		
</div>
</div>
</td>

<td class="msg-panel">
<table class="msg-panel-topbar-submenu">
<tr>
<td>
</td>
</tr>
</table>
<table class="msg-panel-topbar">
<tr>
<td>
<button id="button_reply_" name= "reply_" class="msg-panel-topbar-buttons" style="margin-left:15px;">Reply</button>
<button id="button_forward_" name= "forward_" class="msg-panel-topbar-buttons">Forward</button>
<button id="button_delete_" name= "delete_" class="msg-panel-topbar-buttons">Delete</button>
</td>
</tr>
</table>
<div style="height:265px; overflow:auto; border-bottom:thick; border-bottom-color:#ADADAD; border-bottom-style:solid;">
<table style="width:100%">
<thead>
<tr>
<th style="width:5%">
<input type="checkbox" />
</th>
<th style="width:15%">
From
</th>
<th style="width:15%">
About
</th>
<th style="width:30%">
Purpose
</th>
<th style="width:15%">
Date
</th>
<th style="width:5%">
Belief
</th>
<th style="width:5%">
Consent
</th>
</tr>
</thead>
<tbody>
<tr class="rowstyle1">
<td class="withborder">
<input type="checkbox" />
</td>
<td class="withborder">
Dr. Cox
</td>
<td class="withborder">
Dead
</td>
<td class="withborder">
Creat de-identified Info
</td>
<td class="withborder">
October 25, 2010
</td>
<td class="withborder">
Yes
</td>
<td class="withborder">
No
</td>
</tr>
<tr class="rowstyle2">
<td class="withborder">
<input type="checkbox" />
</td>
<td class="withborder">
Dr. Cox
</td>
<td class="withborder">
Dead
</td>
<td class="withborder">
Creat de-identified Info
</td>
<td class="withborder">
October 25, 2010
</td>
<td class="withborder">
Yes
</td>
<td class="withborder">
No
</td>
</tr>
<tr class="rowstyle1">
<td class="withborder">
<input type="checkbox" />
</td>
<td class="withborder">
Dr. Cox
</td>
<td class="withborder">
Dead
</td>
<td class="withborder">
Creat de-identified Info
</td>
<td class="withborder">
October 25, 2010
</td>
<td class="withborder">
Yes
</td>
<td class="withborder">
No
</td>
</tr>
<tr class="rowstyle2">
<td class="withborder">
<input type="checkbox" />
</td>
<td class="withborder">
Dr. Cox
</td>
<td class="withborder">
Dead
</td>
<td class="withborder">
Creat de-identified Info
</td>
<td class="withborder">
October 25, 2010
</td>
<td class="withborder">
Yes
</td>
<td class="withborder">
No
</td>
</tr>
<tr class="rowstyle1">
<td class="withborder">
<input type="checkbox" />
</td>
<td class="withborder">
Dr. Cox
</td>
<td class="withborder">
Dead
</td>
<td class="withborder">
Creat de-identified Info
</td>
<td class="withborder">
October 25, 2010
</td>
<td class="withborder">
Yes
</td>
<td class="withborder">
No
</td>
</tr>
<tr class="rowstyle2">
<td class="withborder">
<input type="checkbox" />
</td>
<td class="withborder">
Dr. Cox
</td>
<td class="withborder">
Dead
</td>
<td class="withborder">
Creat de-identified Info
</td>
<td class="withborder">
October 25, 2010
</td>
<td class="withborder">
Yes
</td>
<td class="withborder">
No
</td>
</tr>
<tr class="rowstyle1">
<td class="withborder">
<input type="checkbox" />
</td>
<td class="withborder">
Dr. Cox
</td>
<td class="withborder">
Dead
</td>
<td class="withborder">
Creat de-identified Info
</td>
<td class="withborder">
October 25, 2010
</td>
<td class="withborder">
Yes
</td>
<td class="withborder">
No
</td>
</tr>
<tr class="rowstyle2">
<td class="withborder">
<input type="checkbox" />
</td>
<td class="withborder">
Dr. Cox
</td>
<td class="withborder">
Dead
</td>
<td class="withborder">
Creat de-identified Info
</td>
<td class="withborder">
October 25, 2010
</td>
<td class="withborder">
Yes
</td>
<td class="withborder">
No
</td>
</tr>
<tr class="rowstyle1">
<td class="withborder">
<input type="checkbox" />
</td>
<td class="withborder">
Dr. Cox
</td>
<td class="withborder">
Dead
</td>
<td class="withborder">
Creat de-identified Info
</td>
<td class="withborder">
October 25, 2010
</td>
<td class="withborder">
Yes
</td>
<td class="withborder">
No
</td>
</tr>
<tr class="rowstyle2">
<td class="withborder">
<input type="checkbox" />
</td>
<td class="withborder">
Dr. Cox
</td>
<td class="withborder">
Dead
</td>
<td class="withborder">
Creat de-identified Info
</td>
<td class="withborder">
October 25, 2010
</td>
<td class="withborder">
Yes
</td>
<td class="withborder">
No
</td>
</tr>
<tr class="rowstyle1">
<td class="withborder">
<input type="checkbox" />
</td>
<td class="withborder">
Dr. Cox
</td>
<td class="withborder">
Dead
</td>
<td class="withborder">
Creat de-identified Info
</td>
<td class="withborder">
October 25, 2010
</td>
<td class="withborder">
Yes
</td>
<td class="withborder">
No
</td>
</tr>
<tr class="rowstyle2">
<td class="withborder">
<input type="checkbox" />
</td>
<td class="withborder">
Dr. Cox
</td>
<td class="withborder">
Dead
</td>
<td class="withborder">
Creat de-identified Info
</td>
<td class="withborder">
October 25, 2010
</td>
<td class="withborder">
Yes
</td>
<td class="withborder">
No
</td>
</tr>
</tbody>
</table>
</div>
<div style="height:200px">
</div>
</td>
</tr>
</table>
